...
\end{equation}
$$
The operator $\mathcal{N}_\theta$ is a differential operator involving partial derivatives w.r.t. $t$ and $x$ and depends a set of equation parameters $\theta$. The operator $\mathcal{B}$ acts on $\partial \Omega$, the border of $\Omega$. The PDE is said to be stationnary (in time), if $u$ does not change with $t$.
...
**Please note:** The problem of meta-model learning a function $u_{\nu, \theta}$ giving an approximate solution for any values $\theta$ is not yet tackled by the package, although we plan to add this feature in the near future. In this case, the function is learnt over a grid of values $\{\theta_j\}$.
