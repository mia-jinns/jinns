...
from jinns.data import SobolMetaDataGenerator
...
def solve_sobol(
    ...
    get_batch = _get_get_batch(obs_batch_sharding, use_sobol)
    ...
    # If use_sobol true initialize the SobolMetaDataGenerator
    if use_sobol:
        data = SobolMetaDataGenerator(
            key=data.key,
            ...
            ni=data.ni,
            np=param_data.n,
            param_ranges=param_data.param_ranges,
            dim=data.dim,
            tmin=data.tmin,
            tmax=data.tmax,
            domain_batch_size=data.domain_batch_size,
            initial_batch_size=data.initial_batch_size,
            border_batch_size=data.border_batch_size,
            param_batch_size=param_data.param_batch_size,
    ...
    def _one_iteration(carry: main_carry) -> main_carry:
        ...
        if use_sobol:
            if not isinstance(train_data.data, SobolMetaDataGenerator):
                raise ValueError(
                    "When use_sobol is True, data must be a SobolMetaDataGenerator"
                )
...
def _get_get_batch(
    obs_batch_sharding: jax.sharding.Sharding | None, use_sobol: bool | None
) -> Callable[
    [
        AbstractDataGenerator,
    ...
    @jit
    def get_batch_sobol(data, param_data=None, obs_data=None):
        if not isinstance(data, SobolMetaDataGenerator):
            raise ValueError(
                "When Sobol is enabled, data must be a SobolMetaDataGenerator"
            )
        data, batch = data.get_batch()
        return batch, data, param_data, obs_data
