...
from jinns.data import RarDataGenerator
...
def solve_rar(
    ...
    get_batch_init = _get_get_batch(
        obs_batch_sharding, False
    )  # just for the initialization

    # initialize the dict for stored parameter values
    # we need to get a loss_term to init stuff

    ## We use standard DataGenerators to initialize batches
    batch_ini, data, param_data, obs_data = get_batch_init(data, param_data, obs_data)
    _, loss_terms = loss(init_params, batch_ini)

    get_batch = _get_get_batch(obs_batch_sharding, use_rar)
    ...
    # If use_rar is true initialize the RarDataGenerator
    if use_rar:
        data = RarDataGenerator(
            key=key,
            domain_dg=data,
            params_dg=param_data,
    ...
    def _one_iteration(carry: main_carry) -> main_carry:
        ...
        # Update MetaDataGenerator with current model state if RAR is enabled
        if use_rar:
            if not isinstance(train_data.data, RarDataGenerator):
                raise ValueError(
                    "When use_rar is True, data must be a RarDataGenerator"
                )

            redraw_sample = jnp.logical_and(
                jnp.logical_not(train_data.data.in_warmup_phase),
                train_data.data.resampling_counter >= resampling_period,
            )

            def resample_fn(data):
                res_sample = append_param_batch(
                    data.domain_dg.get_batch()[1], data.params_dg.get_batch()[1]
                )

                def residual_vmap(t_x, eq_params):
                    def residual_fn(t_x, eq_params):
                        return loss.dynamic_loss.equation(
                            t_x,
                            loss.u,
                            Params(
                                nn_params=optimization.params.nn_params,
                                eq_params={
                                    "D": jnp.array([eq_params[0]]),
                                    "r": jnp.array([eq_params[1]]),
                                    "g": jnp.array([1.0]),
                                },
                            ),
                        )

                    return jax.vmap(residual_fn, in_axes=(0, 0))(t_x, eq_params)

                # Compute residuals
                current_residuals = residual_vmap(
                    res_sample.domain_batch,
                    jnp.concatenate(
                        [
                            res_sample.param_batch_dict["D"],
                            res_sample.param_batch_dict["r"],
                        ],
                        axis=1,
                    ),
                )
                ...
                # Update data
                data = data.update_current_residuals_and_sample(
                    current_residuals, res_sample
                )
                return data
            ...
            data = jax.lax.cond(redraw_sample, resample_fn, skip_fn, train_data.data)

        # After the modifications we generate the batch
...
def _get_get_batch(
    obs_batch_sharding: jax.sharding.Sharding | None, use_rar: bool | None
) -> Callable[
    [
        AbstractDataGenerator,
    ...
    @jit
    def get_batch_rar(data, param_data=None, obs_data=None):
        if not isinstance(data, RarDataGenerator):
            raise ValueError("When RAR is enabled, data must be a RarDataGenerator")
        data, batch = data.get_batch()
        return batch, data, param_data, obs_data
